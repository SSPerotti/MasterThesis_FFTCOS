...
% GETCHARACTERISTICFUNCTION(model,u,t,r,q,param) 
%
...
elseif strcmp(model,'WMSV') 
    ...
    % List of parameters
    % Matrix M
    M11 = param(1); M12 = param(2);
    M21 = param(3); M22 = param(4);
    % Matrix Q
    R11 = param(5); R12 = param(6);
    R21 = param(7); R22 = param(8);
    % Matrix R
    Q11 = param(9); Q12 = param(10);
    Q21 = param(11); Q22 = param(12);
    % Matrix Sigma0
    Sigma11 = param(13); Sigma12 = param(14);
    Sigma21 = param(15); Sigma22 = param(16);
    % Gindikin
    beta = param(17);
    % Give parameters to characteristic function
    resu = chf_wmsv(u,t,r,q, ...
                    M11,M12,M21,M22, ...
                    R11,R12,R21,R22, ...
                    Q11,Q12,Q21,Q22, ...
...
elseif strcmp(model,'WMSVdiag') 
    ...
    % List of parameters
    % Matrix M
    M11 = param(1); M22 = param(2);
    % Matrix Q
    R11 = param(3); R22 = param(4);
    % Matrix R
    Q11 = param(5); Q22 = param(6);
    % Matrix Sigma0
    Sigma11 = param(7); Sigma22 = param(8);
    % Gindikin
    beta = param(9);
    % Give parameters to characteristic function
    resu = chf_wmsvdiag(u,t,r,q, ...
                        M11,M22, ...
                        R11,R22, ...
                        Q11,Q22, ...
                        Sigma11,Sigma22, ...
...
elseif strcmp(model,'WMSVtrig') 
    ...
    M22 = param(3);
    % Matrix Q
    R11 = param(4); R12 = param(5);
    R22 = param(6);
    % Matrix R
    Q11 = param(7); Q12 = param(8);
    Q22 = param(9);
    % Matrix Sigma0
    Sigma11 = param(10); Sigma12 = param(11);
    Sigma22 = param(12);
    % Gindikin condition
    beta = param(13);
    % Give parameters to characteristic function
    resu = chf_wmsvtrig(u,t,r,q, ...
                        M11,M12,M22, ...
                        R11,R12,R22, ...
                        Q11,Q12,Q22, ...
...
function cf = chf_bates(u,t,r,q,kappa,theta,eta,rho,v0,muJ,sigmaJ,lambda)
    ...
    A = A - lambda*i*u*t*(exp(muJ + 0.5*sigmaJ^2) - 1) ...
        + lambda*t*(exp(i*u*muJ - 0.5*sigmaJ^2*u.^2) - 1);
    % ChF for the Bates model
    cf = exp(A + C * v0);
end

%************************************************************************%
% Here we provide three formulation of WMSV char func
%     - chf_wmsv: It uses all elements of the state matrix
%     - chf_wmsvdiag: It uses only diagonal elements of state matrix
%     - chf_wmsvtrig: It uses upper triangular elements of state matrix
%
%************************************************************************%

% Heston Wishart Stochastic Volatility (full version)
function cf = chf_wmsv(u,t,r,q, ...
    ...
    R = [R11 R12; R21 R22];
    Q = [Q11 Q12; Q21 Q22];
    Sigma0 = [Sigma11 Sigma12; Sigma21 Sigma22];
    i = complex(0,1);
    cf = zeros(1,length(u));   
    for k = 1:length(u) 
        w = u(1,k);
...
% Heston Wishart Stochastic Volatility (diagonal version)
function cf = chf_wmsvdiag(u,t,r,q, ...
                           M11,M22, ...
                           R11,R22, ...
                           Q11,Q22, ...
                           Sigma11,Sigma22, ...
...
% Heston Wishart Stochastic Volatility (triangolar version)
function cf = chf_wmsvtrig(u,t,r,q, ...
                           M11,M12,M22, ...
                           R11,R12,R22, ...
                           Q11,Q12,Q22, ...
                           Sigma11,Sigma12,Sigma22, ...
    ...
    R = [R11 R12; 0 R22];
    Q = [Q11 Q12; 0 Q22];
    Sigma0 = [Sigma11 Sigma12; 0 Sigma22];
    i = complex(0,1);
    cf = zeros(1,length(u));   
    for k = 1:length(u) 
        w = u(1,k);
