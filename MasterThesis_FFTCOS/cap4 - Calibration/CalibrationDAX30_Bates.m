...
r = 5.31/100;   % Tbill 3m USA observed on 20 october 2023
q = 0;
...
lb = [1e-3, 1e-3, 1e-3, -0.99, 1e-3, -1.0, 1e-3, 1e-3];
ub = [10, 1.0, 0.45, 0.99, 1.0, 0.5, 1.0, 20];
...
% =======================================================================
% Implied volatility for DAX30 Index - Bates
% =======================================================================
X = 1:7;
...
% =======================================================================
% Implied volatility DAX30 surface model vs market - Bates
% =======================================================================
figure(2)
...
title('Bates DAX30 Implied Volatility')


% =======================================================================
% DAX30 implied risk neutral distribution - Bates
% =======================================================================
i = complex(0,1);
S0 = S0/100;  % Scale spot price 
y = linspace(120,200,1500);
fXbates = zeros(length(T),length(y));
...
for j = 1:length(T)
    ...
    cf = @(u) getCharacteristicFunction(model,u,T(j),r,q, ...
                                       ...
                                       .* exp(i*u*log(S0));
    fXbates(j,:) = (1./y) .* getRecoveredDensityByCOS(cf,log(y),N,a,b);
    plot(y,fXbates(j,:),'LineWidth',1.5)
